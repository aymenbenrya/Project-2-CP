...
try:
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS marchandise (id_marchandise INT PRIMARY KEY AUTO_INCREMENT,"
...
class marchandise:
    def __init__(self, poids, num_facture,
                 nature, montant, id_dossier, id_emballage, description):
        self.__id_marchandise = 0
        self.__poids = poids
    ...
    def inserer_marchandise(self):
        sql = 'INSERT INTO marchandise(poids,num_facture,nature,montant,id_dossier,id_emballage,description)' \
              + 'VALUES(%s,%s,%s,%s,%s,%s,%s)'
        cur.execute(sql, ( self.__poids, self.__num_facture, self.__nature, self.__montant, self.__id_dossier,
        self.__id_emballage, self.__description))
        db.commit()
        self.__id_marchandise = cur.lastrowid
...
"""
d= Dossier('samir',datetime.now(),datetime.now(),15,'En cours','ali',12,12,'1er intermediaire',
           'titretrans','navire','port','this is SG',
           'lieu entrepot','this is observation','Import','gros','this is an article','eur',1)
d= Dossier('samir',datetime.now(),datetime.now(),15,'En cours','omar',12,13,'2e intermediaire',
           'titretrans','navire','port','this is SG',
           'lieu entrepot','this is observation','Import','gros','this is an article','dollar',1)
d= Dossier('samir',datetime.now(),datetime.now(),15,'En cours','wail',12,15,'3e intermediaire',
           'titretrans','navire','port','this is SG',
           'lieu entrepot','this is observation','Import','gros','this is an article','dollar',1)
