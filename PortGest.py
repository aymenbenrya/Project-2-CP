...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.pushButton_4.clicked.connect(lambda : self.deletdesfact())
        # imprimer
    ...
    def deletdesfact(self):
        try:
            row = self.ui.designationMontantTable.currentRow()
            self.ui.designationMontantTable.removeRow(row)
        except:
            pass
    ...
    def inserer_designation(self):
        try:
            nomdes = self.ui.nom_designation.text()
            typedes = self.ui.typedesignation.text()
            idmarcha = self.ui.num_marchandisedesignation.text()
...
class Setting(QtWidgets.QDialog):
    ...
    def valideBDD(self):
        try:
            ...
            st.close()
            self.close()
            subprocess.call("Python PortGest.py",shell=True)
        except:
            self.ui.tel_8.setStyleSheet("color: rgb(0,0,0);\n"
                                        "border: 1.3px solid rgb(255, 0,0);\n"
                                        "font-size: 14px;")
...
if __name__ == "__main__":
    ...
    if file_size == 0:
        ...
        if not Utilisateur.rechUtilisateur(1):
            Utilisateur('root','root','0000-00-00','00000','root')
